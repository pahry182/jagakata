...
public class ResultSceneController : UIController
{
    ...
    private void DataEntry(List<LeaderboardScoreEntry> entryList)
    {
        ...
        for (int i = 0; i < bc.userScoreDB.results.Count; i++)
        {
            if (string.IsNullOrEmpty(bc.userScoreDB.results[i].nickname)) entryList[i].nickname.text = i + 1 + ". " + "<No Name>";
            else entryList[i].nickname.text = i + 1 + ". " + bc.userScoreDB.results[i].nickname;
